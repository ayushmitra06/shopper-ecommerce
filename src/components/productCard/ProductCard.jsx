        ...
        <section className="text-gray-600 body-font">
            <div className="container px-5 py-8 md:py-16 mx-auto">
                ...
                <div className="flex flex-wrap -m-4">
                    {product
                        .filter((obj) => searchkey === '' || obj.title.toLowerCase().includes(searchkey.toLowerCase()))
                        .filter((obj) => filterType === '' || obj.category.toLowerCase() === filterType.toLowerCase())
                        .filter((obj) => filterPrice === '' || obj.price.toString() === filterPrice)
                        .slice(0, 8)
                        .map((item, index) => {
                                ...
                                <div key={id} onClick={() => window.location.href = `/productinfo/${id}`} className="p-4 md:w-1/4 drop-shadow-lg">
                                    <div className="flex flex-col justify-between h-full border-2 hover:shadow-gray-100 hover:shadow-2xl transition-shadow duration-300 ease-in-out border-gray-200 border-opacity-60 rounded-2xl overflow-hidden" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '' }}>
                                        <div className="flex justify-center cursor-pointer">
                                            <img className="object-cover rounded-2xl w-50 h-50 p-2 hover:scale-110 transition-scale-110 duration-300 ease-in-out" src={imageUrl} alt={title} />
                                        </div>
                                        <div className="p-5 border-t-2">
                                            <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1" style={{ color: mode === 'dark' ? 'white' : '' }}>Shopper</h2>
